...
**起草原则：**
- ⚠️ **所有回复一律用英文起草**，无论客户来信是什么语言。避免翻译导致的表达错误。
- 先回应客户的关切，再给方案
...
但要注意：如果回复内容是独立的新问题（如地址变更、订单查询），按内容意图分类，而不是机械地指给原 Agent。

判断标准：
- 主题含 "Re: Message from Chessnut"、"Re: A shipment from order..." 等
- 查看 conversation 或 notes，找到原始发件人
- ⚠️ **先看回复内容**：如果是客户提供了新信息（地址、EORI号）或新问题（查询订单状态），按正常意图分类
- 只有当回复是关于已有对话的延续时，才指派给原 Agent
...
标题: [邮件主题]
摘要: [邮件内容前120字]
起草: [已发布的 Private Note 纯文本，自然换行，结尾用 ... 截断]
```
